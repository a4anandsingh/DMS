...
class Dms__cate__m extends CI_Model
{
    ...
    function saveData($id, $arrValues){
        ...
        foreach ($arrPathReverse as $key => $value) {
            ...
            $arrSectionName[] = $this->getsectionName($value);
        }
        $HIERARCHY_PATH_TEXT = implode('/', $arrCategoryName);
 $HIERARCHY_PATH_SLUG = implode('/', $arrSectionName);
  $HIERARCHY_PATH_SLUG=str_replace(" ","-",$HIERARCHY_PATH_SLUG);
 $test=  $this->getsectionName($id);
 $s=str_replace(" ","-",strtolower($test));
 
  $final_slug="";
  if(!empty($HIERARCHY_PATH_TEXT))
  {
$final_slug =	str_replace(" ","-",strtolower($HIERARCHY_PATH_TEXT)).'/'.strtolower($s) ; 
  }else{
	$final_slug=  strtolower($s);
  }
	   $arr=array(
            "HIERARCHY_PATH"=>$id.'/'.$parentPath,
            "HIERARCHY_PATH_TEXT" =>$HIERARCHY_PATH_TEXT ,
			"HIERARCHY_SLUG" =>$final_slug
		 );

        $this->db->update($this->dm__category, $arr, array('ID' => $id));

        if($this->db->affected_rows()){
			if($arrValues["CHK_RADIO"]==1)
			{
				$name="Section";
			}else{
				$name="Category";
			}
            array_push($this->message, getMyArray(true, ''.$name.' Name  ' . (($editMode) ? 'Updated' : 'Saved') . '...'));
        }
    ...
    private function showButton($id,$status,$type){
        ...
        /*Start 09-02-2022*/
        if($status==1){
            return getButton('Active', 'Active(2,' . $id . ','.$type.')', 4, '');
        }else{
            return getButton('In-Active', 'Active(1,' . $id . ','.$type.')', 4, '');
        }
        /*End 09-02-2022*/        
    }
    ...
    public function ActiveSection($data){
       // return $this->db->update($this->dm__category,array('STATUS' => $data['STATUS']), array('ID' => $data['ID']));
        $update = $this->db->update($this->dm__category,array('STATUS' => $data['STATUS']), array('ID' => $data['ID']));
        if($update)
...
public  function getsectionName($categoryId){
...
public function getsectionByname($section_name){
	 
		$this-> db->select('*');
		$this ->db->where('CATEGORY_ENG',$section_name); 
        $this -> db -> from('dm__category');     		
		$query = $this ->db->get();			
		//return $this->db->last_query();
		   if($query->num_rows()>0){
            array_push($this->message, getMyArray(0, 'Section Name Already exist'));
		 return createJSONResponse($this->message);
		}else{
		   array_push($this->message, getMyArray(1, ''));
