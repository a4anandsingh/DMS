...
foreach($category as $cat){
  $count=	$this->Dms__frontend__m->get_filesCountByCat($cat['ID']);
  
   $allsub= $this->Dms__frontend__m->get_subcategory($cat['ID']);
	
	foreach($allsub as $sub)
	{
         $subcount = $this->Dms__frontend__m->get_filesCountByCat($sub['ID']);
		 $count = $count + $subcount;
	}
	
	
	?>
<h3 class="dm_title summary">
<a href="<?php echo base_url().'cat_view/'.$section_name.'/'.$cat['SLUG'];?>"><img class="wi" src="<?php echo base_url();?>assets/folder.png" alt="Archive" class="s5_lazyload" style="opacity: 1;"><?php echo $cat['CATEGORY_ENG'];?><small>( <?php echo $count; ?> Files )</small>
</a>
</h3>
<?php } ?>
...
<a  href="<?php echo base_url().$doc["FILE_PATH"];?>" class="data-img row-icon" download><i class="fa fa-download"></i> Download</a> 
<?php if($ext ==="pdf"){?>
<button class="btn" onclick="toggleCommentPopup('<?php echo base_url().$doc["FILE_PATH"];?>')" ><i class="fa fa-eye"></i> View</button>
<?php }else{?>
<button class="btn" onclick="toggleCommentPopupimg('<?php echo base_url().$doc["FILE_PATH"];?>')" ><i class="fa fa-eye"></i> View</button>
<?php } ?>
</div>
<div class="containerbob">
<small>Date added : <?php 		echo  date("d/m/Y", strtotime($doc['UPLOAD_DATE_TIME']));
 ?> </small> 
